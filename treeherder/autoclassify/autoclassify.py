...
def update_db(job, matches, all_matched):
    ...
    classified_failures = {item.id: item for item in
                           ClassifiedFailure.objects.filter(
                               id__in=[match[1].classified_failure_id for match in matches])}
    for item in matches:
        classified_failure = classified_failures[item[1].classified_failure_id]
        matches_by_error[item[1].text_log_error].add((item, classified_failure))
